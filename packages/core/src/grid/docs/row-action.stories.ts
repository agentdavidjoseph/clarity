    ...
    render() {
      return html`
        <cds-grid aria-label="row action datagrid demo" height="360">
          ...
          <cds-grid-row>
            <cds-grid-cell>
              <cds-action popup="row-actions" aria-label="${row.id} actions" @click=${(e: any) => this.select(e.target, row)}></cds-action>
            </cds-grid-cell>
            ${row.cells.map(cell => html`<cds-grid-cell>${cell.label}</cds-grid-cell>`)}
          </cds-grid-row>`)}
          <cds-grid-footer></cds-grid-footer>
        </cds-grid>
        <cds-dropdown id="row-actions" ?hidden=${!this.activeRow} .anchor=${this.anchor} @closeChange=${() => (this.activeRow = null) as any}>
          ${this.grid.rowActions.map(action => html`<cds-button action="flat" size="sm" @click=${() => this.action(action.value, this.activeRow)}>${action.label}</cds-button>`)}
        </cds-dropdown>`;
    }
