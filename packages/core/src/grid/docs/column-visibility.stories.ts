...
export function columnVisibility() {
  ...
  class DemoColumnVisibility extends LitElement {
        ...
        <cds-dropdown id="column-visbility" ?hidden=${!this.columnAnchor} @closeChange=${(): void => this.columnAnchor = null} .anchor=${this.columnAnchor} position="top">
          <cds-checkbox-group layout="vertical">
            ...
            <cds-checkbox>
              ...
              <input type="checkbox" all-columns value=${this.all} @click=${this.toggleAll} .checked=${this.checked(this.all)} ?indeterminate=${this.indeterminate()} />
            </cds-checkbox>
          </cds-checkbox-group>
        </cds-dropdown>`;
    }
    ...
    private selectColumns() {
      this.selectedColumns = Array.from(this.shadowRoot.querySelectorAll<HTMLInputElement>('input[type="checkbox"]:not([all-columns])'))
        .filter(c => c.checked)
