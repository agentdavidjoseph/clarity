      ...
      return html`
        <cds-grid aria-label="Active VM Management" height="360">
          ...
          <cds-grid-column resizable width="180">
            Host <cds-action popup="id-filter" @click=${(e: any) => (this.state = { ...this.state, idFilterAnchor: e.target })} aria-label="column filter options" shape="filter" .pressed=${!!this.state.search}></cds-action>
          </cds-grid-column>
          ...
          <cds-grid-row .selected=${entry.selected}>
            ...
            <cds-grid-cell>
              <cds-action-expand popup="row-detail" action="detail" aria-label="${entry.id} details" .pressed=${this.currentDetail?.id === entry.id} @click=${(e: any) => this.showDetail(entry.id, e.target)}></cds-action-expand>
            </cds-grid-cell>
          ...
          <cds-grid-footer>
            <cds-action popup="column-visibility" @click=${(e: any) => (this.state = { ...this.state, columnsDropdownAnchor: e.target })} aria-label="filter columns" shape="view-columns" .pressed=${!!this.state.search}></cds-action>
            <cds-grid-pagination
            ...
            <div cds-layout="display:screen-reader-only" aria-live="polite" aria-relevant="all" role="status">navigated to page ${this.state.page}</div>
          </cds-grid-footer>
